...
import com.lhx.drysister.imgloader.helper.DiskCacheHelper;
import com.lhx.drysister.imgloader.helper.MemoryCacheHelper;
import com.lhx.drysister.imgloader.helper.NetworkHelper;
import com.lhx.drysister.util.NetworkUtils;
import com.lhx.drysister.util.SizeUtils;
...
/**
 * Description: 图片加载的主要控制类
 * Created by Ly on 2019/8/12.
 */
public class SisterLoader {
    private static final String TAG = SisterLoader.class.getSimpleName();
    ...
    public static final Executor THREAD_POOL_EXECUTOR = new ThreadPoolExecutor(CORE_POOL_SIZE, MAXIMUM_POOL_SIZE,
            KEEP_ALIVE, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), mFactory);
    ...
    private Bitmap loadBitmap(String url, int reqWidth, int reqHeight) {
        ...
        if (bitmap == null && !mDiskHelper.getIsDiskLruCacheCreate()) {
            Log.w(TAG, "磁盘缓存未创建！");
